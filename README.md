...
├── SERVER_DEPLOYMENT.md           # 🚀 Главная инструкция по деплою на сервер
...
├── CREATE_ENV_FILES.md            # Инструкция по созданию конфигурационных файлов
...
- `.env`, `.env.test`, `.env.production` - переменные окружения
- `config.json` - настройки бота (каналы, промпты)
...
2. Настройте конфигурацию:
   - Создайте файл `.env` с переменными окружения (API ключи, номера телефонов)
   - Создайте файл `config.json` с настройками бота (каналы, промпты, интервалы)
   - См. подробную инструкцию: [CREATE_ENV_FILES.md](CREATE_ENV_FILES.md)
   - ⚠️ **Никогда не коммитьте `.env`, `config.json` и `*.session` файлы в git!**
...
**🚀 Главная инструкция по деплою:** [SERVER_DEPLOYMENT.md](SERVER_DEPLOYMENT.md)  
...
```bash
# 1. Создайте директорию и файлы конфигурации
mkdir -p /opt/alpha-parser/app/data
nano /opt/alpha-parser/app/data/.env      # Переменные окружения
nano /opt/alpha-parser/app/data/config.json  # Настройки бота
